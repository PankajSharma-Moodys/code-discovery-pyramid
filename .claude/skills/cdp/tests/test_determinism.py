...
class DeterminismTest(unittest.TestCase):
    ...
    def test_target_repo_scans_reproducibly(self) -> None:
        ...
        with enough elements to be observable at scale.

        Opt-in because it is two ~2m30s scans. `make check TARGET_REPO=...`
        runs it; `PHASE/TARGET.md` pins the repository it runs against.

        Note what is *not* here: a scan of this repository. It would be the
        obvious choice — always available, real, non-fixture — and it is the
        wrong one, because this repository is currently non-deterministic
        (`PHASE/FINDINGS.md` F1: since M0.1 vendored a second copy of the
        fixture, two modules are named `core`, and `cdp/graph.py:180` resolves
        duplicate basenames out of a `set`). Putting it here would make the
        suite fail roughly one run in three, and a gate that flaps is a gate
        every later phase learns to re-run rather than believe. `make
        check-self` demonstrates the failure deliberately instead.
        """
